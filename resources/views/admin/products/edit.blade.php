...
@section('title', 'Edit Product | Shendy')
...
<div class="mb-5">
    ...
    <h1 class="display-5 fw-800 mb-2">Edit Product</h1>
    <p class="text-muted">Modify the details of your product below.</p>
</div>
            ...
            <div class="card border-0 shadow-sm p-4 mb-4 rounded-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" class="form-control form-control-lg border-2" 
                           placeholder="e.g. Premium Wireless Headphones" value="{{ old('name', $product->name) }}" required 
                ...
                <div class="mb-0">
                    ...
                    <textarea name="description" id="description" class="form-control border-2" rows="6" 
                              ...
                              required style="border-radius: 12px;">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <div class="text-danger small mt-1">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group border-2 rounded-3 overflow-hidden">
                            ...
                            <input type="number" name="stock" id="stock" class="form-control border-0 bg-light py-2" 
                                   min="0" placeholder="0" value="{{ old('stock', $product->stock) }}" required>
                        </div>
                        @error('stock')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
        ...
        <div class="col-lg-4">
                ...
                <div class="mb-0">
                    ...
                    <div class="input-group border-2 rounded-3 overflow-hidden shadow-none">
                        ...
                        <input type="number" name="price" id="price" class="form-control border-0 bg-light py-2 fw-bold" 
                               step="0.01" min="0" placeholder="0.00" value="{{ old('price', $product->price) }}" required>
                    </div>
            ...
            <div class="card border-0 shadow-sm p-4 mb-4 rounded-4">
                ...
                <div class="image-upload-wrapper text-center">
                    <div id="image-preview-container" class="mb-3 rounded-4 bg-light d-flex align-items-center justify-content-center overflow-hidden" 
                         style="height: 200px; border: 2px solid #eee;">
                        <img id="image-preview" src="{{ $product->image ? asset($product->image) : '#' }}" 
                             alt="Preview" class="w-100 h-100 object-fit-cover {{ $product->image ? '' : 'd-none' }}">
                    </div>
                    <label for="image" class="btn btn-outline-primary w-100 py-2 fw-bold" style="border-radius: 10px;">
                        <i class="bi bi-arrow-repeat me-1"></i> Change Main Image
                    </label>
            ...
            <div class="card border-0 shadow-sm p-4 rounded-4">
                ...
                <div class="row g-2 mb-4">
                    ...
                    <div class="col-4 position-relative group">
                        <div class="ratio ratio-1x1 rounded-3 overflow-hidden border">
                            <img src="{{ asset($img->image_path) }}" class="object-fit-cover">
                        </div>
                        <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-1 rounded-circle p-1 d-flex align-items-center justify-content-center" 
                ...
                <div class="gallery-upload-wrapper">
                    <label for="images" class="btn btn-light w-100 py-3 border-2 border-dashed d-flex flex-column align-items-center justify-content-center" 
                        ...
                        <span class="text-muted fw-bold text-uppercase small">Add More Images</span>
                    </label>
...
...
            <div class="mt-4">
                <button type="submit" class="btn btn-success w-100 py-3 fw-bold shadow-sm mb-3" style="border-radius: 15px;">
...
@foreach($product->images as $img)
<form id="delete-img-{{ $img->id }}" action="{{ route('admin.products.image.destroy', $img->id) }}" method="POST" class="d-none">
    @csrf
    @method('DELETE')
</form>
@endforeach
